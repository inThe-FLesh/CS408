...
void compute_hash(const ulong W[], ulong* hArr){
    ...
    for (int t = 0; t < 64; t++){
        uint32_t T1 = (h + big_sigma_one(e) + choose(e, f, g) + k[t] + W[t]) % 4294967296;
        uint32_t T2 = (big_sigma_zero(a) + majority(a, b, c)) % 4294967296;
        h = g;
        g = f;
        f = e;
...
__device__ __host__ ulong right_rotation(ulong bits, int n){
    // modulo 32 to ensure that ot can't ever shift more places than there are bits
    n = n % 32;

    u_int32_t shiftedBits = bits >> n;
    u_int32_t rotatedBits = bits << (32 - n);
    return shiftedBits ^ rotatedBits;
}

__global__ void prepare_message_schedule(ulong* schedule, const ulong* paddedBits){
    /*for (int i = 0; i < 16; i++){
        schedule[i] = paddedBits[i];
    }*/
...
// The formula to derive any value for W at position T
void build_message_schedule(ulong* W){
    for (int t = 16; t <= 63; t++){
...
__device__ __host__ ulong sigma_zero(ulong bits){
    ulong sigmaReturn = (right_rotation(bits, 7)) ^ (right_rotation(bits, 18)) ^ (bits >> 3);
    return sigmaReturn;
}

__device__ __host__ ulong sigma_one(ulong bits){
    ulong sigmaReturn = right_rotation(bits, 17) ^ right_rotation(bits, 19) ^ (bits >> 10);
    return sigmaReturn;
}

ulong big_sigma_zero(ulong bits){
    ulong sigmaReturn = right_rotation(bits, 2) ^ right_rotation(bits, 13) ^ right_rotation(bits, 22);
    return sigmaReturn;
}

ulong big_sigma_one(ulong bits){
    ulong sigmaReturn = right_rotation(bits, 6) ^ right_rotation(bits, 11) ^ right_rotation(bits, 25);
    return sigmaReturn;
}
